...
public class ProtectedKMeans extends Classifier {
    ...
    public void classify(MersenneTwister mersenneTwister, ExperimentalData experimentalData, int k) {
        ...
        try {
            ProcessBuilder pb = new ProcessBuilder(PYTHON, PATH, "-k", String.valueOf(k - 1), "-tr",
            ...
            while (scanner.hasNextLine()) {
                ...
                target.get(Integer.parseInt(line[1])).addToGroup(0);
                target.get(Integer.parseInt(line[1])).addToGroup(Integer.parseInt(line[2]) + 1);
            }
        } catch (Exception ex) {
